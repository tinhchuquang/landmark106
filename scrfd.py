...
class SCRFD:
    def __init__(self, model_file=None, session=None, gpu_type='cpu', gpu_id=-1, num_core=2):
        import onnxruntime
        ...
        if self.session is None:
            ...
            # CUDA
            if gpu_type == 'cuda' and 'CUDAExecutionProvider' in available_providers:
                providers = ['CUDAExecutionProvider']
                if gpu_id >= 0:
                    provider_options = [{'device_id': gpu_id}]
            # OpenVINO
            elif gpu_type == 'openvino' and 'OpenVINOExecutionProvider' in available_providers:
                providers = ['OpenVINOExecutionProvider']
                if gpu_id >= 0:
                    provider_options = [{'device_id': 'GPU'}]
                else:
                    provider_options = [{'device_id': 'CPU'}]
            # CPU
            else:
                if num_core > 0:
                    sess_options.intra_op_num_threads = num_core
