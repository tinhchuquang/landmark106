...
from datasets.datasets_pfld import WLFWDatasets
from models.pfld import PFLDInference, AuxiliaryNet
from loss.pfld_loss import PFLDLoss
from utils.utils_pfld import AverageMeter
...
def save_checkpoint(state, filename='checkpoints/pfld/checkpoint.pth'):
...
def main(args):
    ...
    best_val_loss = float('inf')   # Add this before epoch loop
    best_ckpt_path = './checkpoints/pfld/best_model.pth' 
    for epoch in range(args.start_epoch, args.end_epoch + 1):
        ...
        weighted_train_loss, train_loss = train(dataloader, pfld_backbone,
                                            auxiliarynet, criterion,
                                            optimizer, epoch)
        print(f"[Epoch {epoch}] Train loss: {train_loss:.6f} | Weighted loss: {weighted_train_loss:.6f}")

        val_loss = validate(wlfw_val_dataloader, pfld_backbone, auxiliarynet, criterion)
        print(f"[Epoch {epoch}] Eval loss: {val_loss:.6f}")
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_ckpt_path = os.path.join(str(args.snapshot), "best.pth")
            save_checkpoint({
                'epoch': epoch,
                'pfld_backbone': pfld_backbone.state_dict(),
                'auxiliarynet': auxiliarynet.state_dict()
        ...
        last_ckpt_path = os.path.join(str(args.snapshot), "last.pth")
        save_checkpoint({
            'epoch': epoch,
            'pfld_backbone': pfld_backbone.state_dict(),
            'auxiliarynet': auxiliarynet.state_dict()
...
def parse_args():
    ...
    parser.add_argument('--snapshot',
                        default='./checkpoints/pfld',
                        type=str,
                        metavar='PATH')
    parser.add_argument('--log_file',
                        default="./checkpoints/pfld/train.logs",
                        type=str)
    parser.add_argument('--tensorboard',
                        default="./checkpoints/pfld/tensorboard",
                        type=str)
    parser.add_argument(
        '--resume',
        default='',
    ...
    parser.add_argument('--dataroot',
                        default='/data2/tinhcq/WFLW/train_data/list.txt',
                        type=str,
                        metavar='PATH')
    parser.add_argument('--val_dataroot',
                        default='/data2/tinhcq/WFLW/test_data/list.txt',
                        type=str,
                        metavar='PATH')
