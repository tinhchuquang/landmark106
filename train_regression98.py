...
from datasets.datasets_regression import WLFWDatasets
...
def train():
    ...
    model = YOLOv10_Landmark_Flatten(num_landmarks=98)
    ...
    for epoch in range(epochs):
        model.train()
        total_loss = 0
        for img, landmark_gt, _, _ in train_loader:
            img, label = img.cuda(), landmark_gt.cuda()
            pred = model(img)
            loss = criterion(pred, label)
            optimizer.zero_grad()
            loss.backward()
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(model.module.state_dict(), 'checkpoints/yolo/best_model_regression.pth')

    # Save last model
    torch.save(model.module.state_dict(), 'checkpoints/yolo/last_model_regression.pth')
